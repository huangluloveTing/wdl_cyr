    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if displayType == .focusPerson {
            ...
            cell.showInfo(start: info.startProvince+info.startCity,
                          end: info.endProvince+info.endCity, badge: info.total)
            return cell
        }
    }
...
extension FocusResourceVC {
    ...
    func loadResourceByAttentionPath() -> Void {
        self.showLoading()
        BaseApi.request(target: API.findOrderByFollowLine(), type: BaseResponseModel<[FollowLineOrderHallResult]>.self)
            .subscribe(onNext: { [weak self](data) in
                self?.showSuccess()
